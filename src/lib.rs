...
pub enum JType { Object, Array, String, Number, Bool(bool), Null }

#[derive(Debug, PartialEq)]
pub enum JErr { Invalid(usize, char), EndOfInput }

#[derive(Debug, PartialEq, Clone, Copy)]
pub enum JState { Init, StartObject, StartArray, AfterComma, AfterColon, AfterValue, AfterKey }
...
impl <'a>JParse<'a> {
	pub fn new<S: AsRef<str> + ?Sized>(text: &'a S) -> Self {
		...
		Self { text, tokens: Vec::new(), stack: Vec::new(), iter: text.char_indices().peekable(), state: JState::Init } 
	}
	pub fn tokens(&self) -> &[JTok] { &self.tokens }
	pub fn n_tokens(&self) -> usize { self.tokens.len() }
	pub fn value_ok(&self, i: usize, c: char) -> Result<(), JErr> { 
		if self.state == JState::StartObject || self.state == JState::AfterValue || self.state == JState::AfterKey { Err(JErr::Invalid(i, c)) }
		else { Ok(()) }
	}
	fn parse_string(&mut self) -> Result<(), JErr> {
		...
		while let Some((i, c)) = self.iter.next() {
			...
			match c {
				'\"' => {
					self.tokens.push(JTok{tok_type: JType::String, start: start.unwrap(), end: i, size: 0});
					return Ok(())
				},
				'\\' => {
	...
	fn parse_number(&mut self, mut last: usize, c: char) -> Result<usize, JErr> {
		...
		if state == 0 || state == 2 || state >= 5 { Ok (last) }
		else { Err(JErr::EndOfInput) }
	}
	...
	pub fn parse(&mut self) -> Result<usize, JErr> {
		while let Some((i, c)) = self.iter.next() {
			...
			match c {
				...
				'0'..='9' | '-' => {
					...
					self.tokens.push(JTok{tok_type: JType::Number, start: i, end: j, size: 0});					
				...
				'{' => {
					...
					self.tokens.push(JTok{tok_type: JType::Object, start: i, end: i, size: 0});
					self.state = JState::StartObject;
				},
				'}' => {
					if self.state != JState::AfterValue && self.state != JState::StartObject { return Err(JErr::Invalid(i, c)) }
					if let Some((j, old_state)) = self.stack.pop() {
						let tok = &mut self.tokens[j];
						if tok.tok_type != JType::Object { return Err(JErr::Invalid(i, c)) }
						tok.end = i;
						prev_state = old_state;
						tok_ix = j;
						self.state = JState::AfterValue;
					} else { return Err(JErr::Invalid(i, c)) }
				},
				'[' => {
					self.value_ok(i, c)?;
					// Array
					self.stack.push((self.tokens.len(), self.state));
					self.tokens.push(JTok{tok_type: JType::Array, start: i, end: i, size: 0});
					self.state = JState::StartArray;
				},
				']' => {
					if self.state != JState::AfterValue && self.state != JState::StartArray { return Err(JErr::Invalid(i, c)) }
					if let Some((j, old_state)) = self.stack.pop() {
						let tok = &mut self.tokens[j];
						if tok.tok_type != JType::Array { return Err(JErr::Invalid(i, c)) }
						tok.end = i;
						prev_state = old_state;
						tok_ix = j;
						self.state = JState::AfterValue;
				...
				',' => {
					if self.state != JState::AfterValue || self.stack.is_empty() { return Err(JErr::Invalid(i, c)) }
					self.state = JState::AfterComma;
				},
				':' => {
					if self.state != JState::AfterKey || self.tokens.last().unwrap().tok_type != JType::String
						|| self.stack.is_empty() || self.tokens[self.stack.last().unwrap().0].tok_type != JType::Object {
						return Err(JErr::Invalid(i, c)) } 
					self.state = JState::AfterColon;
				},
				'\"' => {
					if self.state == JState::AfterValue { return Err(JErr::Invalid(i, c)) }
					// String
					self.parse_string()?;
					self.state = JState::AfterValue;
				},
				't' if (&self.text[i..]).starts_with("true") => {
					self.value_ok(i, c)?;
					self.tokens.push(JTok{tok_type: JType::Bool(true), start: i, end: i + 3, size: 0});
					let _ = self.iter.nth(2);
					self.state = JState::AfterValue;
				},
				'f' if (&self.text[i..]).starts_with("false") => {
					self.value_ok(i, c)?;
					self.tokens.push(JTok{tok_type: JType::Bool(false), start: i, end: i + 4, size: 0});
					let _ = self.iter.nth(3);
					self.state = JState::AfterValue;
				},
				'n' if (&self.text[i..]).starts_with("null") => {
					self.value_ok(i, c)?;
					self.tokens.push(JTok{tok_type: JType::Null, start: i, end: i + 3, size: 0});
					let _ = self.iter.nth(2);
					self.state = JState::AfterValue;
				},
				_ => return Err(JErr::Invalid(i, c)),
		...
		if !self.stack.is_empty() || self.tokens.is_empty() { Err(JErr::EndOfInput) }
		else { Ok(self.n_tokens()) }
	}
}
...
mod tests {
		...
		fn check(&self, jp: &JParse, ix: usize) {
			...
			match self {
				ExpOut::String(s, size) => {
					...
					assert_eq!(*s, &jp.text[(tok.start as usize)..(tok.end as usize)]);
				},
				ExpOut::Number(s) => {
	...
	fn check_parse(js: &str, res: Result<usize, JErr>, exp: &[ExpOut]) {
		...
		if r.is_ok() && !exp.is_empty() {
			...
			for (i, ex) in exp.iter().enumerate() { ex.check(&jparse, i); }
		}
	}
    ...
    fn test_parse() {
		...
		check_parse(test_json, Ok(13), &[ExpOut::Object(0, 97, 4), ExpOut::String("user", 1), ExpOut::String("johndoe", 0),
			ExpOut::String("admin", 1), ExpOut::Bool(false), ExpOut::String("uid", 1), ExpOut::Number("1000"),
			ExpOut::String("groups", 1), ExpOut::Array(61, 96, 4), ExpOut::String("users", 0), ExpOut::String("wheel", 0),
			ExpOut::String("audio", 0), ExpOut::String("video", 0)]);
    }
	#[test]
	fn test_empty_1() {
	...
	#[test]
	fn test_empty_3() {
		check_parse("[{},{}]", Ok(3), &[ExpOut::Array(0, 6, 2), ExpOut::Object(1, 2, 0), ExpOut::Object(4, 5, 0)]);
	}
	#[test]
	fn test_object_1() {
		check_parse("{\"a\":0}", Ok(3), &[ExpOut::Object(0, 6, 1), ExpOut::String("a", 1), ExpOut::Number("0")]);
	}
	#[test]
	fn test_object_2() {
		check_parse("{\"a\":[]}", Ok(3), &[ExpOut::Object(0, 7, 1), ExpOut::String("a", 1), ExpOut::Array(5, 6, 0)]);
	}
	#[test]
	fn test_object_3() {
		check_parse("{\"a\":{},\"b\":{}}", Ok(5), &[ExpOut::Object(0, 14, 2), ExpOut::String("a", 1), ExpOut::Object(5, 6, 0), ExpOut::String("b", 1), ExpOut::Object(12, 13, 0)]);
	}
	#[test]
	fn test_object_4() {
		check_parse("{\n \"Day\": 26,\n \"Month\": 9,\n \"Year\": 12\n }", Ok(7), &[ExpOut::Object(0, 40, 3), ExpOut::String("Day", 1), ExpOut::Number("26"),
			ExpOut::String("Month", 1), ExpOut::Number("9"), ExpOut::String("Year", 1), ExpOut::Number("12")]);
	}
	#[test]
	fn test_object_5() {
		check_parse("{\"a\": 0, \"b\": \"c\"}", Ok(5), &[ExpOut::Object(0, 17, 2), ExpOut::String("a", 1), ExpOut::Number("0"), ExpOut::String("b", 1), ExpOut::String("c", 0)]);
	}
	#[test]
	fn test_object_6() {
	...
	#[test]
	fn test_array_2() {
		check_parse("[10]", Ok(2), &[ExpOut::Array(0, 3, 1), ExpOut::Number("10")]);
	}
	#[test]
	fn test_array_3() {
		check_parse("{\"a\": 1]", Err(JErr::Invalid(7, ']')), &[]);
	...
	#[test]
	fn test_primitive_1() {
		check_parse("{\"boolVar\" : true }", Ok(3), &[ExpOut::Object(0, 18, 1), ExpOut::String("boolVar", 1), ExpOut::Bool(true)]);
	}
	#[test]
	fn test_primitive_2() {
		check_parse("{\"boolVar\" : false }", Ok(3), &[ExpOut::Object(0, 19, 1), ExpOut::String("boolVar", 1), ExpOut::Bool(false)]);
	}
	#[test]
	fn test_primitive_3() {
		check_parse("{\"nullVar\" : null }", Ok(3), &[ExpOut::Object(0, 18, 1), ExpOut::String("nullVar", 1), ExpOut::Null]);
	}
	#[test]
	fn test_primitive_4() {
		check_parse("{\"intVar\" : 12 }", Ok(3), &[ExpOut::Object(0, 15, 1), ExpOut::String("intVar", 1), ExpOut::Number("12")]);
	}
	#[test]
	fn test_primitive_5() {
		check_parse("{\"floatVar\" : 12.345e4}", Ok(3), &[ExpOut::Object(0, 22, 1), ExpOut::String("floatVar", 1), ExpOut::Number("12.345e4")]);
	}
	#[test]
	fn test_primitive_6() {
	...
	fn test_primitive_11() {
		check_parse("{\"intVar\" : 0 }", Ok(3), &[ExpOut::Object(0, 14, 1), ExpOut::String("intVar", 1), ExpOut::Number("0")]);
	}
	#[test]
	fn test_primitive_12() {
		check_parse("{\"intVar\" : -43.2 }", Ok(3), &[ExpOut::Object(0, 18, 1), ExpOut::String("intVar", 1), ExpOut::Number("-43.2")]);
	}
	#[test]
	fn test_string_1() {
		check_parse("{\"strVar\" : \"hello world\"}", Ok(3), &[ExpOut::Object(0, 25, 1), ExpOut::String("strVar", 1), ExpOut::String("hello world", 0)]);
	}
	#[test]
	fn test_string_2() {
		check_parse("{\"strVar\" : \"escapes: \\/\\r\\n\\t\\b\\f\\\"\\\\\"}", Ok(3), 
			&[ExpOut::Object(0, 39, 1), ExpOut::String("strVar", 1), ExpOut::String("escapes: \\/\\r\\n\\t\\b\\f\\\"\\\\", 0)]);
	}
	#[test]
	fn test_string_3() {
		check_parse("{\"strVar\": \"\"}", Ok(3), &[ExpOut::Object(0, 13, 1), ExpOut::String("strVar", 1), ExpOut::String("", 0)]);
	}
	#[test]
	fn test_string_4() {
		check_parse("{\"a\":\"\\uAbcD\"}", Ok(3), &[ExpOut::Object(0, 13, 1), ExpOut::String("a", 1), ExpOut::String("\\uAbcD", 0)]);
	}
	#[test]
	fn test_string_5() {
		check_parse("{\"a\":\"str\\u0000\"}", Ok(3), &[ExpOut::Object(0, 16, 1), ExpOut::String("a", 1), ExpOut::String("str\\u0000", 0)]);
	}
	#[test]
	fn test_string_6() {
		check_parse("{\"a\":\"\\uFFFFstr\"}", Ok(3), &[ExpOut::Object(0, 16, 1), ExpOut::String("a", 1), ExpOut::String("\\uFFFFstr", 0)]);
	...
	#[test]
	fn test_string_7() {
		check_parse("{\"a\":[\"\\u0280\"]}", Ok(4), &[ExpOut::Object(0, 15, 1), ExpOut::String("a", 1), ExpOut::Array(5, 14, 1), ExpOut::String("\\u0280", 0)]);
	}
	#[test]
	fn test_string_8() {
		check_parse("{\"a\":\"str\\uFFGFstr\"}", Err(JErr::Invalid(13, 'G')), &[]);
	}
	#[test]
	fn test_string_9() {
